...
export default class SchoolsParticipants extends React.Component {
  constructor(props){
    ...
    this.state={
      ...
      checked1: false,
      topCount: 1,
      display: Stores.chartStore.getData().schools.participants,
      blurbText: 'Loading ...'
    }
  }
  ...
  componentWillMount(){
    ...
    let helpTest = Stores.verbiageStore.getData().s_members
    this.setState({
      blurbText: helpTest
    })
  ...
  closeDisplay(){
    ...
    chatStoreData.schools.participants = false
  ...
  showDisplay(){
    let chatStoreData = Stores.chartStore.getData()
    chatStoreData.schools.participants = true
    Stores.chartStore.setData(chatStoreData)
    this.setState({
      display: true
    })
    ...
    AsyncStorage.getItem('@KyndorStore:myId', (err, myId) => {
      if(err){
        ...
        if (myId == a.user_id) {
          ...
          this.props.navToChat.navigate('ChatScreen',{userId:a.user_id, userName:a.name, isGroup:false})
      ...
      <View style={{flex: 1,backgroundColor: (Platform.OS==='ios')?'#f4f4f7':'#fff'}}>
        ...
        <ScrollView style={{backgroundColor: '#fff'}} >
          ...
          <View style={{alignItems: 'center', marginBottom: 20}}>
            <Text style={{fontFamily: "System",paddingBottom: 15, fontSize: 16}} >Can't find your fellow parents?</Text>
            <TouchableOpacity onPress={() => {this.props.navToChat.navigate('SchoolInvite')}}>
