    ...
    .forEach((name) => {
      ...
      if (!value) {
        ...
        if ('email' === name && ((reg.test(value) === false) && (phonereg.test(value) === false))){
          errors[name] = 'Not a valid email or phone number.';
          a=0;
          b=0;
          c=0;
          d=0;
        ...
        if ('referalEmail' === name && ((reg.test(value) === false) && (phonereg.test(value) === false))){
    ...
    return(
      <ImageBackground style={[Styles.loginImageBackgroundStyle, {opacity:(!this.state.loading?1:.7)}]} source={require('../images/background.png')}>
        <View style={Styles.loginContainer} >
          ...
          <View style={{flex: 0.20, justifyContent: 'center', alignContent: 'flex-start'}} >
          ...
          <View style={{flex: 0.80}} >
            ...
            <View style={{flex:0.15}}>
              <TextField
                ...
                label='Referral Email or Phone number'
            ...
            <View style={{flex: 0.25, alignContent: 'center'}}>
              <TouchableOpacity
                disabled={!enabled}
                style={[Styles.loginButton, {marginTop: 20, backgroundColor:(!enabled?'#60678a7a':'#dad4d4')}]}

                onPress={this.onSubmit} >
                <Text style={{color: '#474a82', fontSize: 15,fontWeight:'500'}}> CREATE ACCOUNT </Text>
              </TouchableOpacity>

              <TouchableOpacity style={Styles.loginFacebook} onPress={() => this.handleFacebookLogin()} >
                <View style={{justifyContent: 'flex-start'}}>
                  <Image source={require('../images/fb_icon.png')} />
                </View>
                <Text style={{marginLeft: 20, color: '#fff', fontSize: 15,fontWeight:'500'}}> SIGN UP WITH FACEBOOK</Text>
              </TouchableOpacity>

              <TouchableHighlight
                style={[Styles.registerButton, {marginTop: 0}]} underlayColor={'transparent'}
                // onPress={() => this.props.navigation.goBack()} >
                onPress={() => Stores.rootNavStore.setData('Login')}>
                <Text style={{color: '#FFFFFF', fontSize: 15,fontWeight:'500'}}> - I ALREADY HAVE AN ACCOUNT </Text>
              </TouchableHighlight>
            </View>
            {/* <View style={{flex: 0.15, justifyContent: 'center', alignItems: 'center'}} >
              <Text style={{color: "#a3a5c4"}} >By signing up, I agree</Text>
